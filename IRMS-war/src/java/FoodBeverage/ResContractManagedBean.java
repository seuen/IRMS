...
@ManagedBean
@ViewScoped
public class ResContractManagedBean {
    ...
    private List<Restaurant> allShops = new ArrayList();
    ...
    public List<Contract> getAllContracts() {
        allContracts = cmsb.getAllResContracts();
        return allContracts;
    }
    ...
    public List<Contract> getActiveContracts() {
        activeContracts = cmsb.getAllActiveResContracts();
    ...
    public void navigateShopContracts(ActionEvent event) throws IOException {
        ...
        FacesContext.getCurrentInstance().getExternalContext().redirect("viewMyResContracts.xhtml");
    }
    ...
    public void navigateContractInfo(ActionEvent event) throws IOException {
        ...
        if (getContract().getActivated().compareToIgnoreCase("Inactivated") == 0) {
            FacesContext.getCurrentInstance().getExternalContext().redirect("viewResInactiveContract.xhtml");
        } else {
            FacesContext.getCurrentInstance().getExternalContext().redirect("viewResContractInfo.xhtml");
        }
    }
    ...
    public void saveContract(ActionEvent event) throws IOException {
        if (selectedVenues.isEmpty()) {
            ...
            try {
                ...
                float totalBaseline;
                if (selectedNego.getShopType().equalsIgnoreCase("Restaurant")) {
                    String minRanking = "G";
                ...
                System.out.println(getBaselineRental());
                ...
                if (monthlyRental < baselineRental) {
                    ...
                    Shop s = svmsbl.createRes(selectedNego);
...
//            System.err.println("inside renewContract: before renewContract");
    ...
    public void removeInTable(ActionEvent event) {
        try {
            ...
            if (msg.compareTo("Delete successfully") != 0) {
                ...
                FacesContext.getCurrentInstance().getExternalContext().redirect("viewAllResInactiveContracts.xhtml");
            }
    ...
    public void navigateAllNegotiators(ActionEvent event) throws IOException {
        FacesContext.getCurrentInstance().getExternalContext().redirect("viewAllResNegotiators.xhtml");
    }
...
//        System.out.println(selectedNego.getVenues());
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("selectedNego", selectedNego);
        FacesContext.getCurrentInstance().getExternalContext().redirect("selectResContractVenues.xhtml");
    }
     ...
     * @return the allShops
     */
    public List<Restaurant> getAllShops() {
        allShops = svmsbl.getAllRestaurants();
        return allShops;
    }
     ...
     */
    public void setAllShops(List<Restaurant> allShops) {
    ...
    public List<Contract> getInactiveContracts() {
        inactiveContracts = cmsb.getAllInactiveResContracts();
