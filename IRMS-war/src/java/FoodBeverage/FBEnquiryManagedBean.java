...
@ManagedBean
@SessionScoped
public class FBEnquiryManagedBean {
    ...
    /**
     * Creates a new instance of FBEnquiryManagedBean
     */
    public FBEnquiryManagedBean() {
    }
                    ...
                    if (today.compareTo(getEnquiry().getStartingDate()) < 0) {
                        if (getEnquiry().getStartingDate().after(getEnquiry().getEndingDate())) {
                            ...
                            if (getEnquiry().getStartingDate().compareTo(ca.getTime()) > 0) {
                                ...
                                FacesContext.getCurrentInstance().getExternalContext().redirect("FBEnquiryAddClient.xhtml");
                            }
                        }
                    } else {
                        enquiry = new Enquiry();
    ...
    public void navigatorEnquiry(Enquiry enquiry1) throws IOException {
        ...
        switch (enquiry.getDatetype()) {
            case "day":
                FacesContext.getCurrentInstance().getExternalContext().redirect("FBEnquiryDayDetail.xhtml");
                break;
            case "halfday":
                FacesContext.getCurrentInstance().getExternalContext().redirect("FBEnquiryHalfDayDetail.xhtml");
                break;
            default:
                FacesContext.getCurrentInstance().getExternalContext().redirect("FBEnquiryHourDetail.xhtml");
                break;
        }
    }
    ...
    public List<Enquiry> getUnreadEnquiries() {
        unreadEnquiries = emsbl.listFBUnreadEnquiry();
